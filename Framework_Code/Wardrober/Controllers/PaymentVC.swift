...
//  AuthorizePayments
//
//  Created by Srinivas Peddinti on 2/5/18.
...
class PaymentVC: UIViewController,UITextFieldDelegate {
   ...
   /* func getPaymetAuthenticationCredentials()
    ...
    func updateTokenButton(_ isEnable: Bool) {
        ...
        if isEnable {
            self.getTokenButton.backgroundColor = UIColor.init(red: 0.0/255.0, green: 0.0/255.0, blue: 0.0/255.0, alpha: 1.0)
        } else {
            self.getTokenButton.backgroundColor = UIColor.init(red: 0.0/255.0, green: 0.0/255.0, blue: 0.0/255.0, alpha: 0.2)
        }
    }
