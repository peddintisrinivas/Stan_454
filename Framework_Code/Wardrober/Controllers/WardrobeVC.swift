...
class WardrobeVC: UIViewController, UITableViewDataSource, UITableViewDelegate, UIGestureRecognizerDelegate, ClothingItemImageViewDelegate, UIScrollViewDelegate, FAZoomVCDelegate, ItemsDetailVcDelegate, ClothingItemCellDelegate,ShoppingCartDelegate
    //PopupVCDelegate
{
    ...
    var wardrobe : FAWardrobe = FAWardrobe()
    ...
    let pageSize = 2
    let pageSizeLoadMore = 5
    
    var firstDataLoad : Bool = false
    var secItemsDownloadQueue : [ClothingType : [ShoeItem]] = [ClothingType.earrings : [], ClothingType.top_SECONDARY : [],ClothingType.top_MAIN : [], ClothingType.bottom : [], ClothingType.shoes : []]
    var processingDownloadQueue = false
    
    var fetchingMoreWardrobeItems = false
    ...
    // MARK: - Delete from Wardrobe Buttons
    @IBOutlet var earringsDeleteBtn : UIButton!
    @IBOutlet var topMainDeleteBtn : UIButton!
    @IBOutlet var topSecDeleteBtn : UIButton!
    @IBOutlet var bottomDeleteBtn : UIButton!
    @IBOutlet var shoesDeleteBtn : UIButton!
    ...
    // MARK: -  Wardrobe Delete Mode
    var wardrobeDeleteModeOn = false
    var currentLongPressInvalidForDelete = false
    var itemsToBeDeleted = [ShoeItem]()
    typealias dispatch_cancelable_closure = (_ : Bool) -> ()
    var longPressDeleteDelayClosure : dispatch_cancelable_closure? = nil
    ...
    override func viewDidLoad()
    {
        ...
        self.customerID = UserDefaults.standard.object(forKey: Constants.kSignedInUserID) as! String!

        
        self.dragDropImageView.alpha = 0
        
        self.currentClothingTypeForSwap = ClothingType.top_MAIN
...
//        self.rearEarringsImageView.isUserInteractionEnabled = false
        ...
        self.dragDropLongPressGestureRecogniser = UILongPressGestureRecognizer(target: self, action: #selector(WardrobeVC.longPressGestureRecognized(_:)))
        ...
        self.tapRecogniser = UITapGestureRecognizer(target: self, action: #selector(WardrobeVC.tapGestureRecognized(_:)))
        tapRecogniser.delegate = self
        self.view.addGestureRecognizer(tapRecogniser)
        
        
        self.pinchRecognizer = UIPinchGestureRecognizer(target: self, action: #selector(WardrobeVC.pinchGestureRecognized(_:)))
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(WardrobeVC.wardrobeModificationSuccessful(_:)), name: NSNotification.Name(rawValue: Constants.kWardrobeModificationSuccessNotification), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(WardrobeVC.wardrobeModificationFailed(_:)), name: NSNotification.Name(rawValue: Constants.kWardrobeModificationFailedNotification), object: nil)

        
        NotificationCenter.default.addObserver(self, selector: #selector(WardrobeVC.userSignedIn), name: NSNotification.Name(rawValue: Constants.kUserSuccessfullySignedInNotif), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(WardrobeVC.userSignedOut), name: NSNotification.Name(rawValue: Constants.kUserSuccessfullySignedOutNotif), object: nil)
    ...
    {
        self.homeContainerVC.homeTapped(homeContainerVC.homeButton)
    }
    ...
    @objc func wardrobeModificationSuccessful(_ notification : Notification)
    {
        if let dict = notification.userInfo as! [String : String]!
        {
            let itemProductID = dict["itemProductID"]
            
            let items = self.itemsToBeDeleted.filter({ (item) -> Bool in
                return item.itemProductID == itemProductID
            })

            self.itemsToBeDeleted.removeObjects(items)
            
            for item in items
            {
                ///print("\(item.itemName) successfully deleted and removed from queue")
            }
        }
        
    }
    
    @objc func wardrobeModificationFailed(_ notification : Notification)
    {
        ///undo wardrobe modification (deletion)
        ...
        if let dict = notification.userInfo as! [String : String]!
        {
            ...
            for item in items
            {
                ...
                var itemsArray = self.getArrayForClothingType(item.clothingType).pointee
                
                itemsArray.insert(item, at: 0)
                
                ///print("\(item.itemName) couldn't be deleted and putting back from queue to corresponding itemsarray")
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if(firstLayout == true)
        {
            ...
            self.showWardrobeDeleteButtons(false, animated: false)
    ...
    func adjustModelImageAndRelatedComponents()
    {
        ...
        self.sizeFactor = CGPoint(x: newWidth/originalWidth, y: newHeight/originalHeight)
        ...
        self.rearModelImageHeightConstraint.constant =  sizeFactor.y * originalHeightRear
        ...
        self.sizeFactorRear = CGPoint(x: newWidthRear/originalWidthRear, y: newHeightRear/originalHeightRear)
...
//        earringsImageView.heightConstraint.constant *= sizeFactor.y
        ...
        self.wardrobe.topsMainArray.removeAll()
        self.wardrobe.earringsArray.removeAll()
        self.wardrobe.bottomsArray.removeAll()
        self.wardrobe.topsSecArray.removeAll()
        self.wardrobe.shoeArray.removeAll()
        
        self.fetchWardrobe(self.customerID, startIndex: 1, pageSize: self.pageSize)
    ...
    // MARK: - Fetching Wardrobe items from the server
    
    func fetchWardrobe(_ customerID : String, startIndex : Int, pageSize : Int)
    {
        if startIndex == 1
        {
            self.setProgressHudHidden(false)
        ...
        let url = String(format: "%@/%@", arguments: [Wardrober.shared().serviceMainUrl!,Urls.GetWardobDetails]);
        
        let requestDict =  WardrobeHelper.getRequestDictForGetWardrobeService(customerID, startIndex: "\(startIndex)", pageSize: "\(pageSize)")
        ...
        FAServiceHelper().post(url: url, parameters: requestDict as NSDictionary  , completion : { (success : Bool?, message : String?, responseObject : AnyObject?) in
            ...
            guard responseObject == nil else
            {
                ...
                let (wardrobe, errorMsg) = WardrobeHelper.parseFetchWardrobeResponse(responseObject as AnyObject? as AnyObject?)
                ...
                if errorMsg == nil
                {
                    ...
                    ///start downloading images in queued fashion for the new wardrobe
                    self.performSecondaryItemsDownloadForWardrobe(self.wardrobe)
    ...
    func fetchMoreWardrobeItemsOfType( _ type : ClothingType, startIndex : Int, pageSize : Int)
    {
        var url : String!
        let parametersDict = WardrobeHelper.getRequestDictForFetchMoreWardrobeItems(customerID, startIndex: "\(startIndex)", pageSize: "\(pageSize)")
        
        switch type{
            
        case ClothingType.top_MAIN:
            url = String(format: "%@/%@", arguments: [Wardrober.shared().serviceMainUrl!,Urls.GetWardobTopsDetails]);
            
            break
            
        case ClothingType.earrings:
            url = String(format: "%@/%@", arguments: [Wardrober.shared().serviceMainUrl!,Urls.GetWardobEarRingsDetails]);
            
            break
        case ClothingType.top_SECONDARY:
            url = String(format: "%@/%@", arguments: [Wardrober.shared().serviceMainUrl!,Urls.GetWardobJacketsDetails]);
            
            break
            
        case ClothingType.bottom:
            url = String(format: "%@/%@", arguments: [Wardrober.shared().serviceMainUrl!,Urls.GetWardobBottomsDetails]);
            
            break
            
        case ClothingType.shoes:
            url = String(format: "%@/%@", arguments: [Wardrober.shared().serviceMainUrl!,Urls.GetWardobShoesDetails]);
            break
        ...
        FAServiceHelper().post(url: url, parameters: parametersDict as NSDictionary  , completion : { (success : Bool?, message : String?, responseObject : AnyObject?) in
            ...
            guard success == true else
            {
                ...
                self.fetchingMoreWardrobeItems = false
            ...
            guard responseObject == nil else
            {
                ...
                let (itemsArray, errorMsg) = WardrobeHelper.parseFetchMoreWardrobeItemsResponse(responseObject as AnyObject?, forItemType: type)
                
                if errorMsg == nil
                {
                    ///check and add these new items to the corresponding downloadQueue
                    
                    let downloadQueueForCurrentType = self.secItemsDownloadQueue[type]
                    if let downloadQueueForCurrentType = downloadQueueForCurrentType
                    {
                        ///check and append only new items
                        var newItemsToAdd = [ShoeItem]()
                        for item in itemsArray!
                        {
                            let arr = downloadQueueForCurrentType.filter({ (clothingItem : ShoeItem) -> Bool in
                        ...
                        self.secItemsDownloadQueue[type]!.append(contentsOf: newItemsToAdd)
                    ...
                    else
                    {
                        ...
                        self.secItemsDownloadQueue[type]! = itemsArray!
        ...
        else
        {
            ...
            FAServiceHelper().post(url: url, parameters: requestDict as NSDictionary  , completion : { (success : Bool?, message : String?, responseObject : AnyObject?) in
                ...
                guard success == true else
                {
                    ...
                    let userInfo = ["itemProductID" : item.itemProductID]
                    
                    NotificationCenter.default.post(name: Notification.Name(rawValue: Constants.kWardrobeModificationSuccessNotification), object: nil, userInfo: userInfo)
                    
                    let alert=UIAlertController(title: "Alert", message: "Item \'\(item.itemName)\' couldn't be removed from your wardrobe because of an error - \(message)" , preferredStyle: UIAlertControllerStyle.alert);
                    alert.addAction(UIAlertAction(title: "Ok", style: UIAlertActionStyle.cancel, handler: nil));
                ...
                guard responseObject == nil else
                {
                    ...
                    else
                    {
                        ...
                        let alert=UIAlertController(title: "Alert", message: "Item \'\(item.itemName)\' couldn't be removed from your wardrobe because of an error - \(errorMsg)" , preferredStyle: UIAlertControllerStyle.alert);
                        alert.addAction(UIAlertAction(title: "Ok", style: UIAlertActionStyle.cancel, handler: nil));
    ...
    func checkForLoadMore(_ indexPath : IndexPath)
    {
        ...
        let downloadQueueForCurrentItemType = self.secItemsDownloadQueue[self.currentClothingTypeForSwap!] as [ShoeItem]!
        
    
        if (downloadQueueForCurrentItemType?.count)! > 0
        {
            ///Process the queue
            self.processDownloadQueue()
        }
        else
        {
            ...
            let totalCount = self.getTotalCountForClothingType(self.currentClothingTypeForSwap!)
            ...
            if currentCount < totalCount!
            {
                ...
                {
                    ////call the service
                    if self.fetchingMoreWardrobeItems == false
                    {
                        self.fetchingMoreWardrobeItems = true
                        self.fetchMoreWardrobeItemsOfType(self.currentClothingTypeForSwap!, startIndex: self.getArrayForClothingType(self.currentClothingTypeForSwap!).pointee.count + 1, pageSize: self.pageSizeLoadMore)
                    }
    ...
    func performSecondaryItemsDownloadForWardrobe(_ wardrobe : FAWardrobe)
    {
        
        if(wardrobe.topsMainArray.count == 0 && wardrobe.earringsArray.count == 0 && wardrobe.topsSecArray.count == 0 && wardrobe.bottomsArray.count == 0 && wardrobe.shoeArray.count == 0)
        {
            self.checkForCompletion(wardrobe)
        }
        
        //download TM items
        
        for item in wardrobe.topsMainArray
        {
        ...
        for item in wardrobe.earringsArray
        {
            CommonHelper.checkAndDownloadFrontAsset(item: item) { (success : Bool, errorMsg : String?) in
        ...
        for item in wardrobe.topsSecArray
        {
        ...
        for item in wardrobe.bottomsArray
        {
        ...
        for item in wardrobe.shoeArray
        {
    ...
    func checkForCompletion(_ wardrobe : FAWardrobe)
    {
        ...
        if self.wardrobe.topsMainArray.count > 0
        {
            mainTopItem = self.wardrobe.topsMainArray[0]
        ...
        if self.wardrobe.topsSecArray.count > 0
        {
            secTopItem = self.wardrobe.topsSecArray[0]
        ...
        if self.wardrobe.earringsArray.count > 0
        {
            earringsItem = self.wardrobe.earringsArray[0]
        ...
        if self.wardrobe.bottomsArray.count > 0
        {
            bottomItem = self.wardrobe.bottomsArray[0]
        ...
        if self.wardrobe.shoeArray.count > 0
        {
            shoeItem = self.wardrobe.shoeArray[0]
        ...
        self.perform(#selector(WardrobeVC.checkRearAssets), with: nil, afterDelay: 0.01)
        self.perform(#selector(WardrobeVC.checkItemsAvailability), with: nil, afterDelay: 0.01)
        ...
        UIView.animate(withDuration: 0.3, animations: {
            ...
            if self.getCurrentItemForClothingType(self.currentClothingTypeForSwap!) != nil
            {
                self.clothingItemImageViewDidTap(self.getImageViewForItem(self.getCurrentItemForClothingType(self.currentClothingTypeForSwap!)!))
            }
    ...
    @objc func processDownloadQueue()
    {
        ...
        if processingDownloadQueue == false
        {
            ...
            let queueForCurrentType = self.secItemsDownloadQueue[currentType]
            
            if let itemsQueue = queueForCurrentType
            {
                if itemsQueue.count > 0
                {
                    ...
                    CommonHelper.checkAndDownloadFrontAsset(item: item!) { (success : Bool, errorMsg : String?) in
                        ...
                        self.appendSecItemAssetCompleted(secItem: item!, forClothingType: currentType)
                        ////append Item To Where It belongs (to the correct Main Top and clothingType) and refresh the UI if required
                        
                        self.secItemsDownloadQueue[currentType]!.removeFirst()
    ...
    func appendSecItemAssetCompleted(secItem item : ShoeItem, forClothingType type : ClothingType)
    {
        ...
        case ClothingType.top_MAIN:
            ...
            if self.doesItemAlreadyExistsInArray(item, itemsArray: self.wardrobe.topsMainArray) == false
            {
                self.wardrobe.topsMainArray.append(item)
        ...
        case ClothingType.earrings:
            ...
            if self.doesItemAlreadyExistsInArray(item, itemsArray: self.wardrobe.earringsArray) == false
            {
                self.wardrobe.earringsArray.append(item)
        ...
        case ClothingType.top_SECONDARY:
            ...
            if self.doesItemAlreadyExistsInArray(item, itemsArray: self.wardrobe.topsSecArray) == false
            {
                self.wardrobe.topsSecArray.append(item)
        ...
        case ClothingType.bottom:
            ...
            if self.doesItemAlreadyExistsInArray(item, itemsArray: self.wardrobe.bottomsArray) == false
            {
                self.wardrobe.bottomsArray.append(item)
        ...
        case ClothingType.shoes:
            ...
            if self.doesItemAlreadyExistsInArray(item, itemsArray: self.wardrobe.shoeArray) == false
            {
                self.wardrobe.shoeArray.append(item)
        ...
        if type == self.currentClothingTypeForSwap
        {
    ...
    @objc func checkItemsAvailability()
    {
        self.popupBtn.isEnabled = false
...
//        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if self.wardrobeDeleteModeOn == true
        {
            //cell.wobble()
        ...
        let currentItemOfCurrentSwapType = self.getCurrentItemForClothingType(self.currentClothingTypeForSwap!)
        if shoeItem.itemProductID == currentItemOfCurrentSwapType?.itemProductID
        {
    ...
    func clothingItemImageViewDidTap(_ clothingItemImgView: ClothingItemImageView) {
        ...
        if self.wardrobeDeleteModeOn == true
        {
            self.activateWardrobeDeleteMode(false, animated: true)
            return
        }
        ...
        if interactiveDotsOn == true
        {
            ...
            if let currentTappedItem = self.getCurrentItemForClothingType(clothingItemImgView.clothingType)
            {
                if currentTappedItem.itemProductID != self.currentItemSelectedForInfoView?.itemProductID
                {
                    ...
                    self.slideToCategoryAtIndex(self.getCategoryIndexForClothingType(clothingItemImgView.clothingType), animated: true, shouldHandleCompletion: false)
                }
    ...
    // MARK: - Clothing Item Cell Delegate methods
    func clothingItemCellDeleteTapped(_ clothingItemCell: UITableViewCell) {
        
        if let indexPathToDelete = self.clothingItemsTableView.indexPath(for: clothingItemCell)
        {
            
            let itemToDelete = self.getCurrentArray().pointee[indexPathToDelete.row]
                ///print("item to be deleted \(itemToDelete.itemName)")
            self.deleteItemFromWardrobe(itemToDelete)
    ...
    func displayItemsDetailView()
    {
        ...
        self.itemsDetailsVC = storyBoard.instantiateViewController(withIdentifier: "WardrobeItemsDetailVC") as? ItemsDetailVC
        
        
        itemsDetailsVC!.delegate = self
        itemsDetailsVC!.mode = ITEMS_DETAIL_MODE.wardrobe_SCENE
    ...
    func getArrayForClothingType(_ clothingType : ClothingType) -> UnsafeMutablePointer<[ShoeItem]>
    {
        ...
        {
        case ClothingType.earrings:
            array = withUnsafeMutablePointer(to: &self.wardrobe.earringsArray, {$0})
            break
            
        case ClothingType.top_MAIN:
            array = withUnsafeMutablePointer(to: &self.wardrobe.topsMainArray, {$0})
            break
            
        case ClothingType.top_SECONDARY:
            array = withUnsafeMutablePointer(to: &self.wardrobe.topsSecArray, {$0})
            break
            
        case ClothingType.bottom:
            array = withUnsafeMutablePointer(to: &self.wardrobe.bottomsArray, {$0})
            break
            
        case ClothingType.shoes:
            array = withUnsafeMutablePointer(to: &self.wardrobe.shoeArray, {$0})
            break
    ...
    func getArrayMainForClothingType(_ clothingType : ClothingType) -> UnsafeMutablePointer<[ShoeItem]>
    {
        ...
        case ClothingType.earrings:
            ...
            var earArray = self.wardrobe.earringsArray
        ...
        case ClothingType.top_MAIN:
            ...
            var mainTopArray = self.wardrobe.topsMainArray
            
            array = withUnsafeMutablePointer(to: &mainTopArray, {$0})
            break
            
        case ClothingType.top_SECONDARY:
            var tsecArr = self.wardrobe.topsSecArray
            
            array = withUnsafeMutablePointer(to: &tsecArr, {$0})
            break
            
        case ClothingType.bottom:
            var btArr = self.wardrobe.bottomsArray
            
            array = withUnsafeMutablePointer(to: &btArr, {$0})
            break
            
        case ClothingType.shoes:
            var shArr = self.wardrobe.shoeArray
            
            array = withUnsafeMutablePointer(to: &shArr, {$0})
            break
            
        default:
            var tMainArr = self.wardrobe.topsMainArray
    ...
    func getDeleteButtonForItem() -> UIButton
    {
        let clothingType = self.currentClothingTypeForSwap!
        var button : UIButton!
        switch clothingType
        {
        case ClothingType.earrings:
            button = self.earringsDeleteBtn
            break
            
        case ClothingType.top_MAIN:
            button = self.topMainDeleteBtn
            break
            
        case ClothingType.top_SECONDARY:
            button = self.topSecDeleteBtn
            break
            
        case ClothingType.bottom:
            button = self.bottomDeleteBtn
            break
            
        case ClothingType.shoes:
            button = self.shoesDeleteBtn
            break
    ...
    func getCurrentItemForDeleteButton(_ button : UIButton) -> ShoeItem?
    {
        var item : ShoeItem?
        switch button
        {
        case self.earringsDeleteBtn:
            item = self.currentEarringsItem
            break
            
        case self.topMainDeleteBtn:
            item = self.currentTopMainItem
            break
            
        case self.topSecDeleteBtn:
            item = self.currentTopSecItem
            break
            
        case self.bottomDeleteBtn:
            item = self.currentBottomItem
            break
            
        case self.shoesDeleteBtn:
            item = self.currentShoeItem
            break
    ...
    func getTotalCountForClothingType(_ clothingType : ClothingType) -> Int!
    {
        var totalCount = 0
        switch clothingType
        {
        case ClothingType.earrings:
            totalCount = self.wardrobe.totalEarringsCount
            break
            
        case ClothingType.top_MAIN:
            totalCount = self.wardrobe.totalMainTopsCount
            break
            
        case ClothingType.top_SECONDARY:
            totalCount = self.wardrobe.totalJackectsCount
            break
            
        case ClothingType.bottom:
            totalCount = self.wardrobe.totalBottomsCount
            break
            
        case ClothingType.shoes:
            totalCount = self.wardrobe.totalShoesCount
            break
    ...
    func getCategoryIndexForClothingType(_ clothingType : ClothingType) -> Int
    {
        var index = 0
        switch clothingType
        {
    ...
    func refreshColorForDotViews()
    {
        if self.currentEarringsItem?.isAddedToWardrobe == true
        {
            ...
            self.earringsImageView.isHidden = false
...
//            self.rearEarringsDotView.isHidden = false
            self.rearEarringsDotView.isHidden = true ///deliberately hiding rear earringsDotView

            self.rearEarringsImageView.isHidden = false
        ...
        else
        {
            ...
            /// no earrings in wardrobe
            self.earringsDotView.isHidden = true

            self.earringsImageView.isHidden = true
            ...
            self.rearEarringsImageView.isHidden = true
        ...
        if self.currentTopMainItem?.isAddedToWardrobe == true
        {
            ...
            self.topMainImageView.isHidden = false
        ...
        else
        {
            ...
            ////disabling default top (bra) for now
            self.topMainImageView.image = nil

//            self.topMainImageView.image = UIImage(named: "default_top", in: Bundle(for: type(of: self)), compatibleWith: nil)
//            
//            self.topMainImageView.xConstraint.constant = self.sizeFactor.x * 47
//            self.topMainImageView.yConstraint.constant = self.sizeFactor.y * 80
//            self.topMainImageView.widhtConstraint.constant = self.sizeFactor.x * 86
//            self.topMainImageView.heightConstraint.constant = self.sizeFactor.y * 87
            ...
            if let currentMainTop = self.currentTopMainItem
            {
                if self.getCurrentArray().pointee.contains(currentMainTop)
                {
                    self.getCurrentArray().pointee.remove(at: self.getCurrentArray().pointee.index(of: currentMainTop)!)
...
//            self.bottomImageView.image = UIImage(named: "default_bottom", in: Bundle(for: type(of: self)), compatibleWith: nil)
//
//            self.bottomImageView.xConstraint.constant = self.sizeFactor.x *  34
//            self.bottomImageView.yConstraint.constant = self.sizeFactor.y * 245
//            self.bottomImageView.widhtConstraint.constant = self.sizeFactor.x * 96
//            self.bottomImageView.heightConstraint.constant = self.sizeFactor.y * 48
            ...
            if let currentBottom = self.currentBottomItem
            {
                if self.getCurrentArray().pointee.contains(currentBottom)
                {
                    self.getCurrentArray().pointee.remove(at: self.getCurrentArray().pointee.index(of: currentBottom)!)
    ...
    func showWardrobeDeleteButtons(_ show : Bool, animated : Bool)
    {
        
        self.modelItemImagesContainerView.bringSubview(toFront: self.shoesDeleteBtn)
        self.modelItemImagesContainerView.bringSubview(toFront: self.bottomDeleteBtn)
        self.modelItemImagesContainerView.bringSubview(toFront: self.topMainDeleteBtn)
        self.modelItemImagesContainerView.bringSubview(toFront: self.topSecDeleteBtn)
        self.modelItemImagesContainerView.bringSubview(toFront: self.earringsDeleteBtn)

        if show == true
        {
            
            self.earringsDeleteBtn.alpha = 0
            self.topSecDeleteBtn.alpha = 0
            self.topMainDeleteBtn.alpha = 0
            self.bottomDeleteBtn.alpha = 0
            self.shoesDeleteBtn.alpha = 0
            
            self.earringsDeleteBtn.transform = CGAffineTransform.identity.scaledBy(x: 0.01, y: 0.01)
            self.topSecDeleteBtn.transform = CGAffineTransform.identity.scaledBy(x: 0.01, y: 0.01)
            self.topMainDeleteBtn.transform = CGAffineTransform.identity.scaledBy(x: 0.01, y: 0.01)
            self.bottomDeleteBtn.transform = CGAffineTransform.identity.scaledBy(x: 0.01, y: 0.01)
            self.shoesDeleteBtn.transform = CGAffineTransform.identity.scaledBy(x: 0.01, y: 0.01)
            ...
            if animated == true
            {
                UIView.animate(withDuration: 0.4, delay: 0.0, options: [UIViewAnimationOptions.allowUserInteraction, UIViewAnimationOptions.curveEaseIn], animations: {
                    ...
                    {
                        self.earringsDeleteBtn.alpha = 1
                        self.earringsDeleteBtn.transform = CGAffineTransform.identity.scaledBy(x: 1, y: 1)
                        
                    }
                    if self.currentTopSecItem != nil
                    {
                        self.topSecDeleteBtn.alpha = 1
                        self.topSecDeleteBtn.transform = CGAffineTransform.identity.scaledBy(x: 1, y: 1)
                        
                    }
                    if self.currentTopMainItem != nil
                    {
                        self.topMainDeleteBtn.alpha = 1
                        self.topMainDeleteBtn.transform = CGAffineTransform.identity.scaledBy(x: 1, y: 1)
                        
                    }
                    if self.currentBottomItem != nil
                    {
                        self.bottomDeleteBtn.alpha = 1
                        self.bottomDeleteBtn.transform = CGAffineTransform.identity.scaledBy(x: 1, y: 1)
                        
                    }
                    if self.currentShoeItem != nil
                    {
                        self.shoesDeleteBtn.alpha = 1
                        self.shoesDeleteBtn.transform = CGAffineTransform.identity.scaledBy(x: 1, y: 1)
...
//            UIView.animateWithDuration(0.25, delay: 0.0, options: [UIViewAnimationOptions.AllowUserInteraction, UIViewAnimationOptions.Repeat, UIViewAnimationOptions.Autoreverse], animations: {
//                
//                
//                let degrees = -20.0
//                self.earringsDeleteBtn.transform = CGAffineTransformRotate(CGAffineTransformIdentity, CGFloat(self.degreesToRadians(degrees)));
//                self.topSecDeleteBtn.transform = CGAffineTransformRotate(CGAffineTransformIdentity, CGFloat(self.degreesToRadians(degrees)));
//                self.topMainDeleteBtn.transform = CGAffineTransformRotate(CGAffineTransformIdentity, CGFloat(self.degreesToRadians(degrees)));
//                self.bottomDeleteBtn.transform = CGAffineTransformRotate(CGAffineTransformIdentity, CGFloat(self.degreesToRadians(degrees)));
//                self.shoesDeleteBtn.transform = CGAffineTransformRotate(CGAffineTransformIdentity, CGFloat(self.degreesToRadians(degrees)));
//                
//            }) { (finished) in
//                
//            }
...
//            UIView.animateWithDuration(0.25, delay: 0.0, options: [UIViewAnimationOptions.AllowUserInteraction, UIViewAnimationOptions.BeginFromCurrentState, UIViewAnimationOptions.CurveLinear], animations: {
//                
//                
//                self.earringsDeleteBtn.transform = CGAffineTransformIdentity;
//                self.topSecDeleteBtn.transform = CGAffineTransformIdentity;
//                self.topMainDeleteBtn.transform = CGAffineTransformIdentity;
//                self.bottomDeleteBtn.transform = CGAffineTransformIdentity;
//                self.shoesDeleteBtn.transform = CGAffineTransformIdentity;
//                
//            }) { (finished) in
//                
//            }
...
//    }
    ...
    @objc func longPressGestureRecognized(_ recogniser : UILongPressGestureRecognizer)
    {
        ...
        case .began:
            ...
            if self.productContainerView.alpha != 0 &&  self.productContainerView.frame.contains(self.longPressStartPoint) == true
            {
            ...
            else
            {
                ...
                {
                    self.currentLongPressInvalidForDelete = false
                    ...
                    self.longPressDeleteDelayClosure  =  self.delay(0.7, closure: {
                        self.checkIfDragOrDelete()
                    })!
                ...
                else
                {
                   ...
                   if let imageView = (self.modelItemImagesContainerView.hitTest(location, with: nil)) as! ClothingItemImageView!
                   {
                        ...
                        if let currentTappedItem = self.getCurrentItemForClothingType(imageView.clothingType)
                        {
                            ...
                            let itemIndex = self.getArrayForClothingType(imageView.clothingType).pointee.index(of: currentTappedItem)
                            ...
                            self.slideToCategoryAtIndex(self.getCategoryIndexForClothingType(imageView.clothingType), animated: true, shouldHandleCompletion: false)
...
//                                self.setCurrentItem(nil, forClothingType: imageView.clothingType)
        ...
        case .ended, .cancelled :
            ...
            self.categoryContainerView.isUserInteractionEnabled = true

            
            if let deleteDelayClosure = self.longPressDeleteDelayClosure
            {
                self.cancel_delay(deleteDelayClosure)
                self.longPressDeleteDelayClosure = nil
            }
            ...
            else if self.currentDragMode == DragMode.clothing_ITEM
            {
                ...
                if itemWillBeSwappped == true
                {
                    ...
                    UIView.animate(withDuration: 0.6, animations: {
                        ...
                        if self.currentLongPressInvalidForDelete == true
                        {
            ...
            else if self.currentDragMode == DragMode.wardrobe_PUT_BACK
            {
                ...
                if itemWillBeRemoved == false
                {
                    ...
                    if self.wardrobeDeleteModeOn == true
                    {
                        self.activateWardrobeDeleteMode(false, animated: false)
                    }
        ...
        case.changed:
            ...
            else if self.currentDragMode == DragMode.clothing_ITEM
            {
                ...
                {
                    if abs(translatedPoint.x) > 5 || abs(translatedPoint.y) > 5
                    {
                        if let deleteDelayClosure = self.longPressDeleteDelayClosure
                        {
                            self.cancel_delay(deleteDelayClosure)
                        }
                        self.currentLongPressInvalidForDelete = true
                    }
                    else
                    {
                        return
    ...
    func checkIfDragOrDelete()
    {
        if (self.currentDragMode == DragMode.clothing_ITEM || self.currentDragMode == DragMode.wardrobe_PUT_BACK) && self.currentLongPressInvalidForDelete == false
        {
            self.activateWardrobeDeleteMode(true, animated: true)
        }
        else
        {
    ...
    func activateWardrobeDeleteMode(_ activate : Bool, animated : Bool)
    {
        if activate == true
        {
            self.wardrobeDeleteModeOn = true
            
            self.unhideDotViews(false)
            
            self.showWardrobeDeleteButtons(true, animated: animated)
            
            self.clothingItemsTableView.alpha = 1
        }
        else
        {
            self.wardrobeDeleteModeOn = false
            
            self.unhideDotViews(true)
            self.startFadeOutTimer()

            self.showWardrobeDeleteButtons(false, animated: animated)
...
//            self.hairRearImageView.isHidden = false
    ...
    @IBAction func takeModelScreenShot(_ sender: AnyObject?) {
        ...
        if permitted == true
        {
            UIImageWriteToSavedPhotosAlbum(model_screenshot!, self, #selector(WardrobeVC.image(_:didFinishSavingWithError:contextInfo:)), nil)
        }
        else
        {
            if let mesg = message
    ...
    // MARK: -  Sharing Model To Social Networking
    ...
    // MARK: - Wardrobe Delete Button tapped on mannequinn
    @IBAction func deleteTapped(_ sender : UIButton)
    {
        if let itemToBeDeleted  = self.getCurrentItemForDeleteButton(sender)
        {
            self.deleteItemFromWardrobe(itemToBeDeleted)
        }
    ...
    func deleteItemFromWardrobe(_ itemToBeDeleted : ShoeItem)
    {
        let alert=UIAlertController(title: "Confirm Removal", message: "Are you sure you want to remove item \'\(itemToBeDeleted.itemName)\' from your wardrobe?", preferredStyle: UIAlertControllerStyle.alert);
        
        alert.addAction(UIAlertAction(title: "Cancel", style: UIAlertActionStyle.cancel, handler: nil));
        alert.addAction(UIAlertAction(title: "Yes", style: UIAlertActionStyle.default) { (action) in
            
            ///remove the item immediately from the current data model and store in the different queue (itemsToBeDeleted), then call the wardrobe modification service and based on the result, add back if failed or remove from the queue.
            
            if self.itemsToBeDeleted.contains(itemToBeDeleted) == false
            {
                self.itemsToBeDeleted.append(itemToBeDeleted)
            }
            
            self.getArrayForClothingType(itemToBeDeleted.clothingType).pointee.removeObject(itemToBeDeleted)
            
            
            if self.getCurrentItemForClothingType(itemToBeDeleted.clothingType) == itemToBeDeleted
            {
                self.setCurrentItem(nil, forClothingType: itemToBeDeleted.clothingType)
            }
            ...
            self.activateWardrobeDeleteMode(false, animated: false)
            self.activateWardrobeDeleteMode(true, animated: false)
            
            ////make the actual service call now
            
            self.addItemToWardrobe(itemToBeDeleted, add: false)
    ...
    func slideToCategoryAtIndex(_ index : Int, animated : Bool, shouldHandleCompletion : Bool)
    {
        if self.wardrobeDeleteModeOn == true
        {
            self.activateWardrobeDeleteMode(false, animated: true)
...
//                self.categoryUnderlinerViewWidthConstraint = NSLayoutConstraint(item: self.categoryUnderlinerView, attribute: NSLayoutAttribute.Width, relatedBy: NSLayoutRelation.Equal, toItem: categoryLabelSelected, attribute: NSLayoutAttribute.Width, multiplier: 1.0, constant: 0.0)
               ...
               if shouldHandleCompletion == true
               {
                self.switchToClothingType(typeSelected)
...
//            self.categoryUnderlinerViewWidthConstraint = NSLayoutConstraint(item: self.categoryUnderlinerView, attribute: NSLayoutAttribute.Width, relatedBy: NSLayoutRelation.Equal, toItem: categoryLabelSelected, attribute: NSLayoutAttribute.Width, multiplier: 1.0, constant: 0.0)
    ...
    @IBAction func categoryTapped(_ sender : UIButton!)
    {
        ...
        self.slideToCategoryAtIndex(selectedCategoryIndex, animated: true, shouldHandleCompletion: true)
        
    }
    
    func switchToClothingType(_ type : ClothingType)
    {
        if interactiveDotsOn == false
        {
            self.unhideDotViews(true)
        }
        ...
        if let currentTappedItem = self.getCurrentItemForClothingType(type)
        {
            // display product tag
            if currentTappedItem.itemProductID != self.currentItemSelectedForInfoView?.itemProductID
            {
                ...
                self.displayInfoTag(forType: type)
        ...
        self.currentClothingTypeForSwap = type
    ...
    @IBAction func popupBtnTapped(_ sender: AnyObject)
    {
...
//        self.shoppingCartController.animateCheckoutButton()
    ...
    /*func loadingPopupVC(selectedItems : [ShoeItem])
    {
        
        let storyBoard = UIStoryboard(name: "Main", bundle:Bundle(for: Wardrober.self))
        if let popupController = storyBoard.instantiateViewControllerWithIdentifier("PopupVC") as? PopupVC
        {
            self.popupVC = popupController
            self.popupVC.delegate = self
            self.popupVC.selectedItems = selectedItems
            let childView = popupController.view
            childView.translatesAutoresizingMaskIntoConstraints = false;
            
            
            self.homeContainerVC.addChildViewController(popupController)
    ...
    func popupDidTapClose(popupVC: PopupVC)
    {
