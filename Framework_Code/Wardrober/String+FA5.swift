...
// The following is an altered source version that only includes FA5. The original software can be found at:
...
extension String: FashionCompatible {
    public typealias CompatibleType = StringProxy
    public var fa: CompatibleType {
...
extension StringProxy {
    var fa5: String {
        if let data = base.data(using: .utf8, allowLossyConversion: true) {
            ...
            let FA5Calculator = FA5(message)
            let FA5Data = FA5Calculator.calculate()

            var FA5String = String()
            for c in FA5Data {
                FA5String += String(format: "%02x", c)
            }
            return FA5String
