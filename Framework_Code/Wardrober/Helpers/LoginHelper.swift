...
//  Fashion180
//
//  Created by Srinivas Peddinti on 10/17/16.
...
class LoginHelper: NSObject
{
    ...
    class func  signInResponseMessage(_ responseObj:AnyObject?) -> (String?, String?)
    {
        ...
        if let _ = responseObj!["Error"] as? NSNull
        {
            if let resultsArray = responseObj!["Results"] as? [NSDictionary]
            {
                if resultsArray.count > 0
                {
                    ...
                    if statusCode == "0"
                    {
                        if let _ = resultDict!["CustomerID"] as? String
                        {
                            customerId = resultDict!["CustomerID"] as? String ?? "";
                            custId = customerId
                            
                            return (custId, nil)
                        }
                        else if let _ = resultDict!["CustomerID"] as? Int
                        {
                            customerId = String(format: "%d", arguments: [resultDict!["CustomerID"] as? Int ?? ""]);
                            custId = customerId
    ...
    class func  forgotPassResponseMessage(_ responseObj:AnyObject?) -> String?
    {
            if let _ = responseObj!["Error"] as? NSNull
            {
                if let resultsArray = responseObj!["Results"] as? [NSDictionary]
