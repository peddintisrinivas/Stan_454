...
public extension Notification.Name {
    /**
     ...
     The `object` of this notification is the `FAImageCache` object which sends the notification.
     
     A list of removed hashes (files) could be retrieved by accessing the array under `FashionDiskCacheCleanedHashKey` key in `userInfo` of the notification object you received. By checking the array, you could know the hash codes of files are removed.
     
     The main purpose of this notification is supplying a chance to maintain some necessary information on the cached files. See [this wiki](https://github.com/onevcat/Fashion/wiki/How-to-implement-ETag-based-304-(Not-Modified)-handling-in-Fashion) for a use case on it.
     */
    public static var FashionDidCleanDiskCache = Notification.Name.init("com.onevcat.Fashion.FashionDidCleanDiskCache")
}

/**
Key for array of cleaned hashes in `userInfo` of `FashionDidCleanDiskCacheNotification`.
*/
public let FashionDiskCacheCleanedHashKey = "com.onevcat.Fashion.cleanedHash"
...
/// `FAImageCache` represents both the memory and disk cache system of Fashion. 
/// While a default image cache object will be used if you prefer the extension methods of Fashion, 
/// you can create your own cache object and configure it as your need. You could use an `FAImageCache`
/// object to manipulate memory and disk cache for Fashion.
open class FAImageCache {
    ...
    /// The default cache.
    public static let `default` = FAImageCache(name: "default")
    ...
    */
    public init(name: String,
                path: String? = nil,
                diskCachePathClosure: DiskCachePathClosure = FAImageCache.defaultDiskCachePathClosure)
    {
        
        if name.isEmpty {
            fatalError("[Fashion] You should specify a name for the cache. A cache with empty name is not permitted.")
        }
        
        let cacheName = "com.onevcat.Fashion.FAImageCache.\(name)"
        memoryCache.name = cacheName
        
        diskCachePath = diskCachePathClosure(path, cacheName)
        
        let ioQueueName = "com.onevcat.Fashion.FAImageCache.ioQueue.\(name)"
        ioQueue = DispatchQueue(label: ioQueueName)
        
        let processQueueName = "com.onevcat.Fashion.FAImageCache.processQueue.\(name)"
    ...
    - parameter original:          The original data of the image.
                                   Fashion will use it to check the format of the image and optimize cache size on disk.
    ...
    */
    open func store(_ image: FAImage,
                      original: Data? = nil,
                      forKey key: String,
                      processorIdentifier identifier: String = "",
                      cacheSerializer serializer: FACacheSerializer = DefaultCacheSerializer.default,
                      toDisk: Bool = true,
                      completionHandler: (() -> Void)? = nil)
    {
        
        let computedKey = key.computedKey(with: identifier)
        memoryCache.setObject(image, forKey: computedKey as NSString, cost: image.fa.imageCost)
    ...
    */
    ...
    - parameter options:           Options of retrieving image. If you need to retrieve an image which was 
                                   stored with a specified `FAImageProcessor`, pass the processor in the option too.
    ...
    */
    ...
    open func retrieveImage(forKey key: String,
                     ...
                     completionHandler: ((FAImage?, CacheType) -> ())?) -> RetrieveImageDiskTask?
    {
        ...
        if let image = self.retrieveImageInMemoryCache(forKey: key, options: options) {
            ...
            var sSelf: FAImageCache! = self
            block = DispatchWorkItem(block: {
                // Begin to load image from disk
                if let image = sSelf.retrieveImageInDiskCache(forKey: key, options: options) {
                    if options.backgroundDecode {
                        sSelf.processQueue.async {
                            let result = image.fa.decoded(scale: options.scaleFactor)
    ...
    */
    open func retrieveImageInMemoryCache(forKey key: String, options: FashionOptionsInfo? = nil) -> FAImage? {
        
        let options = options ?? FashionEmptyOptionsInfo
        let computedKey = key.computedKey(with: options.processor.identifier)
        
        return memoryCache.object(forKey: computedKey as NSString) as? FAImage
    }
    ...
    - returns: The image object if it is cached, or `nil` if there is no such key in the cache.
    */
    open func retrieveImageInDiskCache(forKey key: String, options: FashionOptionsInfo? = nil) -> FAImage? {
        ...
        ioQueue.async {
            ...
            DispatchQueue.main.async {
                ...
                if URLsToDelete.count != 0 {
                    ...
                    NotificationCenter.default.post(name: .FashionDidCleanDiskCache, object: self, userInfo: [FashionDiskCacheCleanedHashKey: cleanedHashes])
                }
    ...
    */
    @objc public func backgroundCleanExpiredDiskCache() {
        ...
        guard let sharedApplication = Fashion<UIApplication>.shared else { return }
...
extension FAImageCache {
  
    func diskImage(forComputedKey key: String, serializer: FACacheSerializer, options: FashionOptionsInfo) -> FAImage? {
    ...
    func cacheFileName(forComputedKey key: String) -> String {
        if let ext = self.pathExtension {
          return (key.fa.fa5 as NSString).appendingPathExtension(ext)!
        }
        return key.fa.fa5
    }
}
...
extension UIApplication: FashionCompatible { }
extension Fashion where Base: UIApplication {
